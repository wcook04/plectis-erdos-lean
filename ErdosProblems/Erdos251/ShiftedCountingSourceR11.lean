...
/-!
# Relocated finite shifted-gap count

The finite proof is reused from the exact source linked in the owned paper:
ShiftedGapCountingR9.lean at d058b9150218ae23d2cfd975580088d6167e9da8.
The R11 full-coverage audit receipt binds this file at SHA-256 c3656197 before
this comment-only status correction; all declarations and proofs below are unchanged.
The fresh namespace removes unrelated parent dependencies. The four-prime
sieve estimate remains an explicit, unproved analytic premise and is NOT
used to establish the Schlage-Puchta boundary.
-/
noncomputable section
open Filter Topology Finset
namespace ErdosProblems.Erdos251.PaperR11.PrimeSource
open Nonconcentration

def gapWindow (n k : ℕ) : ℕ := ∑ i ∈ range k, primeGap0 (n + i)

theorem gapWindow_identity (n k : ℕ) : prime0 n + gapWindow n k = prime0 (n + k) := by
  induction k with
...
theorem short_shifted_matches_le_quad (h N H : ℕ) (hh : 2 ≤ h) (r : ℤ) :
  ...
  · intro n hn
    ...
    have ep : prime0 n + primeGap0 n = prime0 (n + 1) := by unfold primeGap0; omega
...
/-- This analytic estimate is displayed as an unresolved premise in the paper. -/
...
/-- For h=1, the two middle prime positions coincide; d<s is impossible. -/
theorem adjacent_middle_positions_coincide (n : ℕ) :
    prime0 n + primeGap0 n = prime0 (n + 1) := by
  unfold primeGap0
  have hm := prime0_mono_step n
  omega

#print axioms shifted_count_bound
#print axioms separated_zeroDensity_of_quad_sieve
