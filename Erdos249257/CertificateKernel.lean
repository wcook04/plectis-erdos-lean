...
-- `Finset.dvd_lcm` uses the semigroup divisibility instance; arithmetic lemmas
-- below use the natural-number instance. Both carry the same multiplier witness.
private theorem nat_dvd_of_semigroup_dvd {a b : Nat}
    (h : @Dvd.dvd Nat (semigroupDvd (α := Nat)) a b) : a ∣ b := by
  rcases h with ⟨c, hc⟩
  exact ⟨c, hc⟩
...
theorem exists_canonical_witness_selector_row
  ...
  have hn0a' : (S.max' hS_ne).factorization p ≤ (F.lcm id).factorization p :=
    factorization_le_of_dvd_ne_zero hL0
      (by simpa using nat_dvd_of_semigroup_dvd (Finset.dvd_lcm hn0F))
...
theorem exact_order_witness_supplies_PrimeComponentWitness
  ...
  have hden_dvd : ∀ m ∈ F, b ^ m - 1 ∣ b ^ L - 1 := by
    ...
    exact Nat.pow_sub_one_dvd_pow_sub_one b
      (by simpa [hL_eq] using nat_dvd_of_semigroup_dvd (Finset.dvd_lcm hmF))
...
theorem finiteErdosSum_eq_commonNumerator_div_commonDenominator
  ...
  have hn_dvd : n ∣ F.lcm id := by
    simpa using nat_dvd_of_semigroup_dvd (Finset.dvd_lcm hn)
...
theorem lcm_Icc_lt_den_erdosPartialSum
  ...
  have h2dvd : (2 : Nat) ∣ (Finset.Icc 1 N).lcm id := by
    simpa using nat_dvd_of_semigroup_dvd (Finset.dvd_lcm h2mem)
...
theorem lcm_image_factorial_succ (k : ℕ) (hk : 1 ≤ k) :
  ...
  · have hmem : Nat.factorial k
    ...
    simpa using nat_dvd_of_semigroup_dvd (Finset.dvd_lcm hmem)
...
theorem lcm_gap_hypothesis_fails_full_support :
  ...
  have h0 : ∀ᶠ k in atTop,
    ...
    have hd1 : (k - 1) ∣ ((Finset.range k).image (fun i => i + 1)).lcm id := by
      simpa using nat_dvd_of_semigroup_dvd (Finset.dvd_lcm hmem1)
    have hd2 : k ∣ ((Finset.range k).image (fun i => i + 1)).lcm id := by
      simpa using nat_dvd_of_semigroup_dvd (Finset.dvd_lcm hmem2)
