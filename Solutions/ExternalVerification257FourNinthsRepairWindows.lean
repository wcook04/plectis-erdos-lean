...
/-!
# Source transport for four-ninths square-root repair windows

The compared statements are transported from
`ErdosProblems.Erdos257.exists_repair_in_sqrt_window`,
`four_ninths_mem_iff_repairCofinal`,
`four_ninths_mem_iff_repair_sqrt_windows`, and
`four_ninths_not_mem_of_strict_sqrt_window`.
This is not the tetraprime `4/9` package.
-/
