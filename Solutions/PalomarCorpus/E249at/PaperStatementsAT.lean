...
theorem certifiedKill_diagonal_all_imported_through_t64 :
    ∀ t ∈ diagonalPincerCertificateScalesThroughT64,
      certifiedKill (periodLcm t) (periodLcm t) (diagonalPincerKillDepthThroughT64 t) := @Erdos249257.TotientTailPeriodKiller.certifiedKill_diagonal_all_imported_through_t64

theorem certifiedKill_diagonal_t64 :
    certifiedKill (periodLcm 64) (periodLcm 64) 93 := @Erdos249257.TotientTailPeriodKiller.certifiedKill_diagonal_t64

theorem irrational_totientSeries_iff_cofinalDirectedLcmCertificateSupply :
    Irrational (∑' n : ℕ, (Nat.totient n : ℝ) / 2 ^ n) ↔
      CofinalDirectedLcmCertificateSupply := @Erdos249257.irrational_totientSeries_iff_cofinalDirectedLcmCertificateSupply
...
theorem clean_lcm_ray_factorisation (t j q : ℕ) (hdvd : j ∣ periodLcm t)
    ...
    Nat.totient (q * periodLcm t + j) = Nat.totient j * Nat.totient (q * (periodLcm t / j) + 1) := @ErdosProblems.Erdos249.PaperCompleteR20.clean_lcm_ray_factorisation t j q hdvd hclean

theorem finite_carry_test_sound (h N K : ℕ)
    (htest : ∀ z : ℤ, |z| ≤ (N + h + 1 : ℤ) →
      ∃ i : ℕ, i ≤ K ∧ (N + i + h + 2 : ℤ) ≤ |carryOrbit h N z i|) :
    totientTail (N + h) - totientTail N ∉ Set.range ((↑) : ℤ → ℝ) := @ErdosProblems.Erdos249.PaperCompleteR20.finite_carry_test_sound h N K htest

theorem finite_carry_true_orbit (h N : ℕ) (z : ℤ)
    (hz : (z : ℝ) = totientTail (N + h) - totientTail N) (i : ℕ) :
    (carryOrbit h N z i : ℝ) = totientTail (N + i + h) - totientTail (N + i) ∧ := @ErdosProblems.Erdos249.PaperCompleteR20.finite_carry_true_orbit h N z hz i
...
theorem lcm_grid_flatness
    ...
    ∃ t₁ : ℕ, ∀ t, t₁ ≤ t → ∀ q m : ℕ, 0 < q →
      totientTail ((q + m) * periodLcm t) - totientTail (q * periodLcm t)
        ∈ Set.range ((↑) : ℤ → ℝ) := @ErdosProblems.Erdos249.PaperCompleteR20.lcm_grid_flatness hrat

theorem lcm_grid_fractional_parts
    (hrat : ¬ Irrational (∑' n : ℕ, (Nat.totient n : ℝ) / 2 ^ n)) :
    ∃ t₁ : ℕ, ∀ t, t₁ ≤ t → ∀ q m : ℕ, 0 < q →
      Int.fract (totientTail ((q + m) * periodLcm t)) =
        Int.fract (totientTail (q * periodLcm t)) := @ErdosProblems.Erdos249.PaperCompleteR20.lcm_grid_fractional_parts hrat

theorem lcm_grid_multiplier_positive (t q m L : ℕ)
    (hc : certifiedKill (m * periodLcm t) (q * periodLcm t) L) : 0 < m := @ErdosProblems.Erdos249.PaperCompleteR20.lcm_grid_multiplier_positive t q m L hc

theorem lcm_grid_supply_iff :
    Irrational (∑' n : ℕ, (Nat.totient n : ℝ) / 2 ^ n) ↔
      ∀ t₀ : ℕ, ∃ t, t₀ ≤ t ∧ ∃ q m L : ℕ, 0 < q ∧
        certifiedKill (m * periodLcm t) (q * periodLcm t) L := @ErdosProblems.Erdos249.PaperCompleteR20.lcm_grid_supply_iff
...
theorem short_lcm_window_nondivisor (t j : ℕ) (ht : 1 ≤ t) (hj : 1 ≤ j)
    ...
    ∃ p a : ℕ, Nat.Prime p ∧ 1 ≤ a ∧ j = p ^ a ∧ t < j := @ErdosProblems.Erdos249.PaperCompleteR20.short_lcm_window_nondivisor t j ht hj hlt hnd

theorem totient_scaled_truncation_error (h N L : ℕ) : := @ErdosProblems.Erdos249.PaperCompleteR20.totient_scaled_truncation_error h N L
...
theorem abs_tail_diff_scaled_sub_window_le (h N L : ℕ) : := @ErdosProblems.Erdos249.PaperCompleteR21.abs_tail_diff_scaled_sub_window_le h N L

theorem actualLcmRawErrorRadius_tendsto_zero (a : ℕ) :
    Filter.Tendsto (fun q : ℕ => actualLcmRawErrorRadius a q) Filter.atTop (nhds 0) := @ErdosProblems.Erdos249.PaperCompleteR21.actualLcmRawErrorRadius_tendsto_zero a

theorem actualLcmTailOrbit_eq_tail_difference (a : ℕ) :
    actualLcmTailOrbit a =
      totientTail (2 * periodLcm (2 ^ a)) - totientTail (periodLcm (2 ^ a)) := @ErdosProblems.Erdos249.PaperCompleteR21.actualLcmTailOrbit_eq_tail_difference a
    ...
    totientTail (2 * periodLcm (2 ^ a)) - totientTail (periodLcm (2 ^ a))
      = (2 : ℝ) ^ periodLcm (2 ^ a) * ((2 : ℝ) ^ periodLcm (2 ^ a) - 1)
          ...
          - ((totientPrefix (2 * periodLcm (2 ^ a)) : ℝ)
              - (totientPrefix (periodLcm (2 ^ a)) : ℝ)) := @ErdosProblems.Erdos249.PaperCompleteR21.actualLcmTailOrbit_global_to_local a

theorem actualLcmTailOrbit_pos {a : ℕ} (ha : 8 ≤ a) :
    0 < totientTail (2 * periodLcm (2 ^ a)) - totientTail (periodLcm (2 ^ a)) := @ErdosProblems.Erdos249.PaperCompleteR21.actualLcmTailOrbit_pos a ha

theorem actualLcm_corridor_pos {a : ℕ} (ha : 8 ≤ a) :
    (∀ J : ℕ, J + (a + 6) < 2 * 2 ^ a →
        0 < totientTail (2 * periodLcm (2 ^ a) + J)
              - totientTail (periodLcm (2 ^ a) + J)) ∧
      0 < totientTail (2 * periodLcm (2 ^ a)) - totientTail (periodLcm (2 ^ a)) := @ErdosProblems.Erdos249.PaperCompleteR21.actualLcm_corridor_pos a ha
      ...
      ((e : ℝ) = totientTail (2 * periodLcm (2 ^ a) + J + K)
        ...
        ∧ ((2 : ℤ) ^ K - ((2 * periodLcm (2 ^ a) + J + K + 2 : ℕ) : ℤ)
            ...
            ∧ windowDiscrepancy (periodLcm (2 ^ a)) (periodLcm (2 ^ a) + J) K
                  % (2 : ℤ) ^ K < (2 : ℤ) ^ K)
        ∧ (2 : ℤ) ^ K ∣
            windowDiscrepancy (periodLcm (2 ^ a)) (periodLcm (2 ^ a) + J) K + e := @ErdosProblems.Erdos249.PaperCompleteR21.actualLcm_integral_forces_topEdgeResidue_paper a J K ha hshort hroom hint

theorem actualLcm_tailDiff_shift_pos_paper {a J : ℕ} (ha : 8 ≤ a)
    (hshort : J + (a + 6) < 2 * 2 ^ a) :
    0 < totientTail (2 * periodLcm (2 ^ a) + J)
          - totientTail (periodLcm (2 ^ a) + J) := @ErdosProblems.Erdos249.PaperCompleteR21.actualLcm_tailDiff_shift_pos_paper a J ha hshort
...
theorem certifiedKill_of_fullDepth_phase_separation (h N : ℕ)
    (hsep : ∀ k : ℤ,
      2 * ((N : ℝ) + 2 * h + 2) / 2 ^ h < := by
  apply ErdosProblems.Erdos249.PaperCompleteR21.certifiedKill_of_fullDepth_phase_separation <;> assumption
...
theorem corridor_height_lt_letter {a j : ℕ} (ha : 8 ≤ a) (hj : 0 < j)
    (hjlt : j < 2 * 2 ^ a) :
    (periodLcm (2 ^ a) : ℤ) < 8 * (2 ^ a : ℤ) * lcmRayArithmeticLetter (2 ^ a) j := @ErdosProblems.Erdos249.PaperCompleteR21.corridor_height_lt_letter a j ha hj hjlt

theorem corridor_letter_pos {a j : ℕ} (ha : 8 ≤ a) (hj : 0 < j)
    (hjlt : j < 2 * 2 ^ a) :
    0 < lcmRayArithmeticLetter (2 ^ a) j := @ErdosProblems.Erdos249.PaperCompleteR21.corridor_letter_pos a j ha hj hjlt
...
theorem diagonal_certificate_unfolded (a L : ℕ) :
    certifiedKill (periodLcm (2 ^ a)) (periodLcm (2 ^ a)) L ↔
      (((2 * periodLcm (2 ^ a) + L + 2 : ℕ) : ℤ) <
        ...
        windowDiscrepancy (periodLcm (2 ^ a)) (periodLcm (2 ^ a)) L % 2 ^ L <
          2 ^ L - ((2 * periodLcm (2 ^ a) + L + 2 : ℕ) : ℤ)) := @ErdosProblems.Erdos249.PaperCompleteR21.diagonal_certificate_unfolded a L
...
theorem exists_periodLcm_strict_jump_ge_paper (t₀ : ℕ) :
  ...
  apply ErdosProblems.Erdos249.PaperCompleteR21.exists_periodLcm_strict_jump_ge_paper <;> assumption
    ...
    j ∣ periodLcm (2 ^ a)
      ...
      ∧ (∀ q : ℕ, 0 < q → q ≤ 3 →
          Nat.gcd j (q * (periodLcm (2 ^ a) / j) + 1) = 1
            ...
            ∧ Even (Nat.totient (q * (periodLcm (2 ^ a) / j) + 1))) := @ErdosProblems.Erdos249.PaperCompleteR21.fixedRank_cleanWindow_structure a j ha hj hsq
...
theorem four_tail_error_bound (H p L : ℕ) : := @ErdosProblems.Erdos249.PaperCompleteR21.four_tail_error_bound H p L
...
theorem integral_carry_strictly_between {a q : ℕ} (ha : 8 ≤ a)
    ...
    0 < carryOrbit (periodLcm (2 ^ a)) (periodLcm (2 ^ a)) z (2 * q + 1) ∧
      carryOrbit (periodLcm (2 ^ a)) (periodLcm (2 ^ a)) z (2 * q + 1) <
        ((2 * periodLcm (2 ^ a) + 2 * q + 3 : ℕ) : ℤ) := @ErdosProblems.Erdos249.PaperCompleteR21.integral_carry_strictly_between a q ha hshort z hz
...
theorem integral_tail_forces_upper_endpoint_residue {a J K : ℕ} (ha : 8 ≤ a)
    ...
    (2 : ℤ) ^ K - ((2 * periodLcm (2 ^ a) + J + K + 2 : ℕ) : ℤ) <
      ...
      windowDiscrepancy (periodLcm (2 ^ a)) (periodLcm (2 ^ a) + J) K % (2 : ℤ) ^ K <
        (2 : ℤ) ^ K := @ErdosProblems.Erdos249.PaperCompleteR21.integral_tail_forces_upper_endpoint_residue a J K ha hshort hroom d hd

theorem irrational_iff_diagonal_orbit_nonintegrality :
    Irrational (∑' n : ℕ, (Nat.totient n : ℝ) / 2 ^ n) ↔
      ∀ a₀ : ℕ, ∃ a, a₀ ≤ a ∧
        totientTail (2 * periodLcm (2 ^ a)) - totientTail (periodLcm (2 ^ a)) ∉
          Set.range ((↑) : ℤ → ℝ) := @ErdosProblems.Erdos249.PaperCompleteR21.irrational_iff_diagonal_orbit_nonintegrality
...
theorem irrational_of_diagonal_orbit_separation_supply
  ...
  apply ErdosProblems.Erdos249.PaperCompleteR21.irrational_of_diagonal_orbit_separation_supply <;> assumption
...
theorem irrational_of_four_tail_supply
    ...
    Irrational (∑' n : ℕ, (Nat.totient n : ℝ) / 2 ^ n) := @ErdosProblems.Erdos249.PaperCompleteR21.irrational_of_four_tail_supply hsupply
...
theorem irrational_of_logarithmicDepth_diagonal_supply
    ...
    Irrational (∑' n : ℕ, (Nat.totient n : ℝ) / 2 ^ n) := @ErdosProblems.Erdos249.PaperCompleteR21.irrational_of_logarithmicDepth_diagonal_supply hsupply
