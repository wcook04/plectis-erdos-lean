...
import Solutions.PalomarCorpus.E249_19.Statement
...
theorem positive_rational_difference_lower_bound
    ...
    (1 : ℝ) /
      ...
      (whole : ℝ) - (pfx : ℝ) := @Erdos249257.positive_rational_difference_lower_bound whole pfx hpositive

theorem finite_carry_candidate_count (h N : ℕ) :
    (Finset.Icc (-(N + h + 1 : ℤ)) (N + h + 1)).card = 2 * (N + h + 1) + 1 := @ErdosProblems.Erdos249.PaperCompleteR20.finite_carry_candidate_count h N

theorem moebius_three_values (d : ℕ) :
    ArithmeticFunction.moebius d = -1 ∨ ArithmeticFunction.moebius d = 0 ∨
      ArithmeticFunction.moebius d = 1 := @ErdosProblems.Erdos249.PaperCompleteR20.moebius_three_values d

theorem rational_cross_numerator_positive {u v : ℚ} (h : u < v) :
    1 ≤ v.num * (u.den : ℤ) - u.num * (v.den : ℤ) := @ErdosProblems.Erdos249.PaperCompleteR20.rational_cross_numerator_positive u v h

theorem rational_difference_exact (u v : ℚ) :
    (v : ℝ) - u =
      ((v.num * (u.den : ℤ) - u.num * (v.den : ℤ) : ℤ) : ℝ) /
        ((v.den : ℝ) * u.den) := @ErdosProblems.Erdos249.PaperCompleteR20.rational_difference_exact u v

theorem rational_error_denominator_bound {u v : ℚ} {ε : ℝ}
    (h : u < v) (he : (v : ℝ) - u ≤ ε) :
    1 / ((u.den : ℝ) * ε) ≤ v.den := by
  apply ErdosProblems.Erdos249.PaperCompleteR20.rational_error_denominator_bound <;> assumption

theorem binaryCyclotomicLayer_eventual_instance (m : ℕ) (hm : 0 < m) :
    EventualBoundedDegreeOrderConsumer binaryCyclotomicLayer m 1 := @ErdosProblems.Erdos249.PaperCompleteR21.binaryCyclotomicLayer_eventual_instance m hm

theorem binaryCyclotomic_allPrime_form_fails :
    ((Polynomial.cyclotomic 6 ℤ).eval 2).natAbs = 3 ∧ ¬ (6 ∣ 3 - 1) ∧
      ¬ BoundedDegreeOrderConsumer binaryCyclotomicLayer 2 1 := @ErdosProblems.Erdos249.PaperCompleteR21.binaryCyclotomic_allPrime_form_fails

theorem boundedDegreeOrderConsumer_unfolded (C : ℕ → ℕ) (m d : ℕ) :
    BoundedDegreeOrderConsumer C m d ↔
      ∀ q p : ℕ, q.Prime → p.Prime → p ∣ C (m * q) →
        ∃ k : ℕ, 1 ≤ k ∧ k ≤ d ∧ m * q ∣ p ^ k - 1 := @ErdosProblems.Erdos249.PaperCompleteR21.boundedDegreeOrderConsumer_unfolded C m d

theorem boundedOrder_witness_iff_orderOf_le {n p d : ℕ} (hn : 0 < n) (hp : 0 < p)
    (hcop : Nat.Coprime p n) :
    (∃ k : ℕ, 1 ≤ k ∧ k ≤ d ∧ n ∣ p ^ k - 1) ↔ orderOf ((p : ℕ) : ZMod n) ≤ d := @ErdosProblems.Erdos249.PaperCompleteR21.boundedOrder_witness_iff_orderOf_le n p d hn hp hcop
...
theorem complementDenominator_dvd_scalar
    ...
    (hscaled : ((c : ℚ) * x).den ∣ H) :
    ((x.den / H : ℕ) : ℤ) ∣ c := @ErdosProblems.Erdos249.PaperCompleteR21.complementDenominator_dvd_scalar x c H _hHpos hH hscaled

theorem composite_dilation_defect_eq_zero_of_prime_support (A : Set ℕ) {a x : ℕ}
    (ha : a ∈ A) (hAprime : ∀ d ∈ A, d.Prime) :
    compositeDilationDefect A a x = 0 := @ErdosProblems.Erdos249.PaperCompleteR21.composite_dilation_defect_eq_zero_of_prime_support A a x ha hAprime

theorem composite_dilation_defect_univ_six_one :
    ((6 * 1 : ℕ).divisors.filter
        fun d => (d ∈ (Set.univ : Set ℕ) ∧ ¬ d ∣ 1 ∧ d ≠ 6)) = ({2, 3} : Finset ℕ) := @ErdosProblems.Erdos249.PaperCompleteR21.composite_dilation_defect_univ_six_one

theorem composite_dilation_defect_univ_six_one_card :
    compositeDilationDefect (Set.univ : Set ℕ) 6 1 = 2 := @ErdosProblems.Erdos249.PaperCompleteR21.composite_dilation_defect_univ_six_one_card

theorem coprimeLattice_gcd_layer_total {r : ℝ} (hr0 : 0 ≤ r) (hr1 : r < 1) :
    ∑' g : ℕ, (∑' p : ℕ × ℕ,
        if 0 < p.1 ∧ 0 < p.2 ∧ Nat.Coprime p.1 p.2 then (r ^ (g + 1)) ^ (p.1 + p.2) else 0)
      = (r / (1 - r)) ^ 2 := @ErdosProblems.Erdos249.PaperCompleteR21.coprimeLattice_gcd_layer_total r hr0 hr1

theorem coprimeLattice_gcd_layer_total_eq_one_iff {r : ℝ} (hr0 : 0 ≤ r) (hr1 : r < 1) :
    (∑' g : ℕ, (∑' p : ℕ × ℕ,
        if 0 < p.1 ∧ 0 < p.2 ∧ Nat.Coprime p.1 p.2 then (r ^ (g + 1)) ^ (p.1 + p.2) else 0))
        = 1 ↔ r = 1 / 2 := @ErdosProblems.Erdos249.PaperCompleteR21.coprimeLattice_gcd_layer_total_eq_one_iff r hr0 hr1

theorem coprimeLattice_halfOpen_half_eq_series :
    (∑' p : ℕ × ℕ,
        if 0 < p.1 ∧ Nat.Coprime p.1 p.2 then (1 / 2 : ℝ) ^ (p.1 + p.2) else 0)
      = ∑' n : ℕ, (Nat.totient n : ℝ) / 2 ^ n := @ErdosProblems.Erdos249.PaperCompleteR21.coprimeLattice_halfOpen_half_eq_series

theorem coprimeLattice_halfOpen_sum {r : ℝ} (hr0 : 0 ≤ r) (hr1 : r < 1) :
    (∑' p : ℕ × ℕ, if 0 < p.1 ∧ Nat.Coprime p.1 p.2 then r ^ (p.1 + p.2) else 0)
      = ∑' n : ℕ, (Nat.totient (n + 1) : ℝ) * r ^ (n + 1) := @ErdosProblems.Erdos249.PaperCompleteR21.coprimeLattice_halfOpen_sum r hr0 hr1

theorem coprimeLattice_lambert_half_eq_one :
    (∑' p : ℕ × ℕ,
        if 0 < p.1 ∧ 0 < p.2 ∧ Nat.Coprime p.1 p.2 then
          (1 / 2 : ℝ) ^ (p.1 + p.2) / (1 - (1 / 2 : ℝ) ^ (p.1 + p.2)) else 0) = 1 := @ErdosProblems.Erdos249.PaperCompleteR21.coprimeLattice_lambert_half_eq_one

theorem coprimeLattice_lambert_identity {r : ℝ} (hr0 : 0 ≤ r) (hr1 : r < 1) :
    (∑' p : ℕ × ℕ,
        if 0 < p.1 ∧ 0 < p.2 ∧ Nat.Coprime p.1 p.2 then
          r ^ (p.1 + p.2) / (1 - r ^ (p.1 + p.2)) else 0)
      = (r / (1 - r)) ^ 2 := @ErdosProblems.Erdos249.PaperCompleteR21.coprimeLattice_lambert_identity r hr0 hr1

theorem coprimeLattice_lambert_rational (s : ℚ) (hs0 : 0 ≤ s) (hs1 : s < 1) :
    ∃ v : ℚ, (∑' p : ℕ × ℕ,
        if 0 < p.1 ∧ 0 < p.2 ∧ Nat.Coprime p.1 p.2 then
          (s : ℝ) ^ (p.1 + p.2) / (1 - (s : ℝ) ^ (p.1 + p.2)) else 0)
      = (v : ℝ) := @ErdosProblems.Erdos249.PaperCompleteR21.coprimeLattice_lambert_rational s hs0 hs1
...
theorem coprimeLattice_plain_half_eq_series_sub_half :
    (∑' p : ℕ × ℕ,
      ...
      (∑' n : ℕ, (Nat.totient n : ℝ) / 2 ^ n) - 1 / 2
        ≠ ∑' n : ℕ, (Nat.totient n : ℝ) / 2 ^ n := @ErdosProblems.Erdos249.PaperCompleteR21.coprimeLattice_plain_half_eq_series_sub_half

theorem coprimeLattice_positive_sum {r : ℝ} (hr0 : 0 ≤ r) (hr1 : r < 1) :
    (∑' p : ℕ × ℕ, if 0 < p.1 ∧ 0 < p.2 ∧ Nat.Coprime p.1 p.2 then r ^ (p.1 + p.2) else 0)
      = (∑' n : ℕ, (Nat.totient (n + 1) : ℝ) * r ^ (n + 1)) - r := @ErdosProblems.Erdos249.PaperCompleteR21.coprimeLattice_positive_sum r hr0 hr1

theorem coprimeLattice_removed_pair (a b : ℕ) :
    ((0 < a ∧ Nat.Coprime a b) ∧ ¬ (0 < a ∧ 0 < b ∧ Nat.Coprime a b))
      ↔ (a = 1 ∧ b = 0) := @ErdosProblems.Erdos249.PaperCompleteR21.coprimeLattice_removed_pair a b

theorem coprime_of_dvd_pow_sub_one {n p k : ℕ} (hp : 0 < p) (hk : 1 ≤ k)
    (hdvd : n ∣ p ^ k - 1) : Nat.Coprime p n := @ErdosProblems.Erdos249.PaperCompleteR21.coprime_of_dvd_pow_sub_one n p k hp hk hdvd

theorem cyclotomic_four_eval (x : ℤ) :
    (Polynomial.cyclotomic 4 ℤ).eval x = x ^ 2 + 1 := @ErdosProblems.Erdos249.PaperCompleteR21.cyclotomic_four_eval x

theorem cyclotomic_four_two_pow_eq_cyclotomic_two (h : ℕ) :
    (Polynomial.cyclotomic 4 ℤ).eval ((2 : ℤ) ^ h) = 2 ^ (2 * h) + 1 ∧
      (Polynomial.cyclotomic 2 ℤ).eval ((2 : ℤ) ^ (2 * h)) = 2 ^ (2 * h) + 1 := @ErdosProblems.Erdos249.PaperCompleteR21.cyclotomic_four_two_pow_eq_cyclotomic_two h

theorem cyclotomic_layer_prime_order_decomposition_paper {n p : ℕ}
    (hn : 0 < n) (hp : p.Prime)
    (hpdvd : p ∣ ((Polynomial.cyclotomic n ℤ).eval 2).natAbs) :
    ∃ a : ℕ, n = p ^ a * orderOf ((2 : ℕ) : ZMod p) := @ErdosProblems.Erdos249.PaperCompleteR21.cyclotomic_layer_prime_order_decomposition_paper n p hn hp hpdvd

theorem cyclotomic_three_eval_two_not_dvd_doubling_chain :
    (Polynomial.cyclotomic 3 ℤ).eval 2 = 7 ∧
      orderOf ((2 : ℕ) : ZMod 7) = 3 ∧
      (∀ j : ℕ, ¬ (3 ∣ 2 ^ j)) ∧
      (∀ j : ℕ, ¬ (7 ∣ 2 ^ 2 ^ j - 1)) := @ErdosProblems.Erdos249.PaperCompleteR21.cyclotomic_three_eval_two_not_dvd_doubling_chain

theorem cyclotomic_three_two_pow (h : ℕ) :
    (Polynomial.cyclotomic 3 ℤ).eval ((2 : ℤ) ^ h) = 2 ^ (2 * h) + 2 ^ h + 1 := @ErdosProblems.Erdos249.PaperCompleteR21.cyclotomic_three_two_pow h

theorem den_lower_bound_of_positive_error {S u : ℚ} (hlt : u < S) {ε : ℝ}
    (herr : (S : ℝ) - (u : ℝ) ≤ ε) :
    (1 : ℝ) / ((u.den : ℝ) * ε) ≤ (S.den : ℝ) := by
  apply ErdosProblems.Erdos249.PaperCompleteR21.den_lower_bound_of_positive_error <;> assumption

theorem den_mul_abs_sub_ge_one_div_den {q u : ℚ} (hqu : q ≠ u) :
    (1 : ℝ) / (q.den : ℝ) ≤ (u.den : ℝ) * |(q : ℝ) - (u : ℝ)| := @ErdosProblems.Erdos249.PaperCompleteR21.den_mul_abs_sub_ge_one_div_den q u hqu

theorem denominator_bound_tendsto_atTop_iff {f : ℕ → ℝ} (hpos : ∀ N, 0 < f N) :
    Filter.Tendsto (fun N => 1 / f N) Filter.atTop Filter.atTop ↔
      Filter.Tendsto f Filter.atTop (nhds 0) := @ErdosProblems.Erdos249.PaperCompleteR21.denominator_bound_tendsto_atTop_iff f hpos

theorem doublingMap_iterate_apply (α : ℝ) (N : ℕ) :
    (fun x : ℝ => 2 * x)^[N] α = 2 ^ N * α := by
  apply ErdosProblems.Erdos249.PaperCompleteR21.doublingMap_iterate_apply <;> assumption

theorem dvd_pow_sub_one_iff_orderOf_dvd {n p k : ℕ} (hn : 0 < n) (hp : 0 < p) :
    n ∣ p ^ k - 1 ↔ orderOf ((p : ℕ) : ZMod n) ∣ k := @ErdosProblems.Erdos249.PaperCompleteR21.dvd_pow_sub_one_iff_orderOf_dvd n p k hn hp

theorem dyadicMixedGuard_iff_twoBitBand (A : ℤ) (b : ℕ) :
    DyadicMixedGuard A b ↔
      ((2 : ℤ) ^ b ≤ A % (2 : ℤ) ^ (b + 2)
        ∧ A % (2 : ℤ) ^ (b + 2) < 3 * (2 : ℤ) ^ b) := @ErdosProblems.Erdos249.PaperCompleteR21.dyadicMixedGuard_iff_twoBitBand A b

theorem dyadicScale_unique_in_open_interval {B : ℤ} {m₁ m₂ : ℕ}
    (h₁ : B < (2 : ℤ) ^ m₁) (h₁' : (2 : ℤ) ^ m₁ < 2 * B)
    (h₂ : B < (2 : ℤ) ^ m₂) (h₂' : (2 : ℤ) ^ m₂ < 2 * B) :
    m₁ = m₂ := by
  apply ErdosProblems.Erdos249.PaperCompleteR21.dyadicScale_unique_in_open_interval <;> assumption

theorem dyadic_prefix_denominator_bound_vacuous (N : ℕ) :
    (1 : ℝ) / ((2 : ℝ) ^ N * (((N : ℝ) + 2) / (2 : ℝ) ^ N)) = 1 / ((N : ℝ) + 2) ∧
      1 / ((N : ℝ) + 2) ≤ 1 := @ErdosProblems.Erdos249.PaperCompleteR21.dyadic_prefix_denominator_bound_vacuous N
...
theorem eventual_orderConsumer_conclusions {C : ℕ → ℕ} {m d : ℕ}
    ...
    FinitePrimeSupportEscape C m ∧
      (∀ hsupply : PrimeRayLayerSupply C m, UnboundedPrimeDivisorSupply C m) := @ErdosProblems.Erdos249.PaperCompleteR21.eventual_orderConsumer_conclusions C m d hm horder
...
theorem exists_clean_cyclotomic_anchor_paper (h N₀ : ℕ) (hh : 0 < h) :
  ...
  apply ErdosProblems.Erdos249.PaperCompleteR21.exists_clean_cyclotomic_anchor_paper <;> assumption

theorem exists_prime_totient_shift_four_mul_congr_two_mod_four
    (h B : ℕ) (hh : 0 < h) :
    ∃ p : ℕ, B < p ∧ p.Prime ∧
      ((Nat.totient (p + 4 * h) : ℤ) - (Nat.totient p : ℤ)) ≡ (2 : ℤ) [ZMOD 4] := @ErdosProblems.Erdos249.PaperCompleteR21.exists_prime_totient_shift_four_mul_congr_two_mod_four h B hh

theorem exists_rational_parityComparison :
    ∃ c : ℕ → ℕ, ParityComparisonProperties c ∧
      ¬ Irrational (∑' n : ℕ, (c n : ℝ) / 2 ^ n) := @ErdosProblems.Erdos249.PaperCompleteR21.exists_rational_parityComparison

theorem extremal_order_curvature_ne_zero {H j : ℕ} (hH : 1 ≤ H)
    (hextremal : MiddleRankTotientExtremal H j) :
    (Nat.totient (3 * H + j) : ℤ) - 2 * Nat.totient (2 * H + j) + Nat.totient (H + j) ≠ 0 := @ErdosProblems.Erdos249.PaperCompleteR21.extremal_order_curvature_ne_zero H j hH hextremal
...
theorem extremal_order_curvature_neg {H j : ℕ} (hH : 1 ≤ H)
    ...
    (Nat.totient (3 * H + j) : ℤ) - 2 * Nat.totient (2 * H + j) + Nat.totient (H + j) < 0 := @ErdosProblems.Erdos249.PaperCompleteR21.extremal_order_curvature_neg H j hH hleft hright

theorem extremal_order_curvature_pos {H j : ℕ} (hH : 1 ≤ H)
    (hmin : Nat.totient (2 * H + j) < min (Nat.totient (H + j)) (Nat.totient (3 * H + j))) :
    0 < (Nat.totient (3 * H + j) : ℤ) - 2 * Nat.totient (2 * H + j) + Nat.totient (H + j) := @ErdosProblems.Erdos249.PaperCompleteR21.extremal_order_curvature_pos H j hH hmin

theorem farey_gap_paper {a b c d r s : ℤ} (hb : 0 < b) (hd : 0 < d)
    (hdet : b * c - a * d = 1) (hleft : a * s < r * b) (hright : r * d < c * s) :
    b + d ≤ s := @ErdosProblems.Erdos249.PaperCompleteR21.farey_gap_paper a b c d r s hb hd hdet hleft hright
    ...
    (∀ q : ℕ, 1 ≤ q → q ≤ 79639646646701375323355774875831053 →
      ...
      ¬ ((79639646646701375323355774875831054 *
              ((∑ r ∈ Finset.Icc 1 240, Nat.totient (1 + r) * 2 ^ (240 - r))
                % 2 ^ 240)) % 2 ^ 240
            + 79639646646701375323355774875831054 * 243 < 2 ^ 240) := @ErdosProblems.Erdos249.PaperCompleteR21.farey_window_1_240_exact_range

theorem farey_window_1_240_range_magnitude :
    (796 : ℕ) * 10 ^ 32 ≤ 79639646646701375323355774875831053 ∧
      (79639646646701375323355774875831053 : ℕ) < 797 * 10 ^ 32 := @ErdosProblems.Erdos249.PaperCompleteR21.farey_window_1_240_range_magnitude

theorem gapCertificate_window_1_240 (q : ℕ) (hq : 0 < q)
    (hqQ : q ≤ 79639646646701375323355774875831053) :
    GapCertificate 1 240 q := @ErdosProblems.Erdos249.PaperCompleteR21.gapCertificate_window_1_240 q hq hqQ
...
theorem gapCheck_window_1_240_first_failure_paper :
    IsFirstGapFailure
      ...
      240 243 79639646646701375323355774875831054 := @ErdosProblems.Erdos249.PaperCompleteR21.gapCheck_window_1_240_first_failure_paper
...
theorem gapCheck_window_1_240_paper (q : ℕ) (hq : 0 < q)
    ...
    (q * 1299094806818720335611738031537456208600423915562142231419225521361164904)
        % 2 ^ 240 + q * 243 < 2 ^ 240 := @ErdosProblems.Erdos249.PaperCompleteR21.gapCheck_window_1_240_paper q hq hqQ

theorem gapCheck_window_one_excludes (K q : ℕ) (hq : 0 < q)
    (hcert : (q * ((∑ r ∈ Finset.Icc 1 K, Nat.totient (1 + r) * 2 ^ (K - r)) % 2 ^ K))
        % 2 ^ K + q * (1 + K + 2) < 2 ^ K) :
    ∀ a : ℤ, (∑' n : ℕ, (Nat.totient n : ℝ) / 2 ^ n) ≠ (a : ℝ) / (q : ℝ) := @ErdosProblems.Erdos249.PaperCompleteR21.gapCheck_window_one_excludes K q hq hcert

theorem gapFareyBound_window_1_240 (q : ℕ) (hq : 0 < q)
    (hqQ : q ≤ 79639646646701375323355774875831053) :
    (q * 1299094806818720335611738031537456208600423915562142231419225521361164904)
        % 2 ^ 240 + q * 243 < 2 ^ 240 := @ErdosProblems.Erdos249.PaperCompleteR21.gapFareyBound_window_1_240 q hq hqQ
...
theorem irrational_of_den_mul_error_product_tendsto_zero
    ...
    Irrational x := by
  apply ErdosProblems.Erdos249.PaperCompleteR21.irrational_of_den_mul_error_product_tendsto_zero <;> assumption
...
theorem irrational_of_den_mul_error_tendsto_zero {x : ℝ} {u : ℕ → ℚ}
    ...
    (h0 : Filter.Tendsto (fun k => ((u k).den : ℝ) * |x - ((u k : ℚ) : ℝ)|)
      Filter.atTop (nhds 0)) :
    Irrational x := @ErdosProblems.Erdos249.PaperCompleteR21.irrational_of_den_mul_error_tendsto_zero x u hne h0

theorem irrational_of_dirichlet_gap {x : ℝ}
    (h : ∀ Q : ℤ, 1 ≤ Q → ∃ m z : ℤ,
      0 < |(m : ℝ) * x - (z : ℝ)| ∧ |(m : ℝ) * x - (z : ℝ)| < 1 / (Q : ℝ)) :
    Irrational x := @ErdosProblems.Erdos249.PaperCompleteR21.irrational_of_dirichlet_gap x h

theorem irrational_of_near_integer_base_powers (b₀ : ℕ) (hb : 2 ≤ b₀) {ξ : ℝ}
    (h : ∀ q : ℕ, 0 < q → ∃ (n : ℕ) (z : ℤ),
      0 < |(b₀ : ℝ) ^ n * ξ - (z : ℝ)| ∧ |(b₀ : ℝ) ^ n * ξ - (z : ℝ)| < 1 / (q : ℝ)) :
    Irrational ξ := by
  apply ErdosProblems.Erdos249.PaperCompleteR21.irrational_of_near_integer_base_powers <;> assumption

theorem irrational_of_near_integer_multiples {ξ : ℝ}
    (h : ∀ q : ℕ, 0 < q → ∃ m z : ℤ,
      0 < |(m : ℝ) * ξ - (z : ℝ)| ∧ |(m : ℝ) * ξ - (z : ℝ)| < 1 / (q : ℝ)) :
    Irrational ξ := by
  apply ErdosProblems.Erdos249.PaperCompleteR21.irrational_of_near_integer_multiples <;> assumption
...
theorem irrational_of_unbounded_window_one_gapCheck (g : ℕ → ℕ)
    ...
    (hcheck : ∀ K q : ℕ, 0 < q → q ≤ g K →
      (q * ((∑ r ∈ Finset.Icc 1 K, Nat.totient (1 + r) * 2 ^ (K - r)) % 2 ^ K))
        % 2 ^ K + q * (1 + K + 2) < 2 ^ K) :
    Irrational (∑' n : ℕ, (Nat.totient n : ℝ) / 2 ^ n) := @ErdosProblems.Erdos249.PaperCompleteR21.irrational_of_unbounded_window_one_gapCheck g hg hcheck
...
theorem irrational_totientSeries_of_block_cosine_gap
    (hgap : ∀ h : ℕ, 1 ≤ h → ∀ X₀ : ℕ, ∃ X : ℕ, max X₀ 1 ≤ X ∧
      (∑ N ∈ Finset.Ico X (2 * X),
        ...
        ≤ (89 / 100 : ℝ) * X) :
    Irrational (∑' n : ℕ, (Nat.totient n : ℝ) / 2 ^ n) := @ErdosProblems.Erdos249.PaperCompleteR21.irrational_totientSeries_of_block_cosine_gap hgap

theorem irrational_totientSeries_of_fareyGapExclusionUnbounded
    (hsup : FareyGapExclusionUnbounded) :
    Irrational (∑' n : ℕ, (Nat.totient n : ℝ) / 2 ^ n) := @ErdosProblems.Erdos249.PaperCompleteR21.irrational_totientSeries_of_fareyGapExclusionUnbounded hsup
...
theorem mersenneLayer_orderConsumer_instance :
    BoundedDegreeOrderConsumer (fun n => 2 ^ n - 1) 1 1 ∧
      ∀ q p : ℕ, q.Prime → p.Prime → p ∣ 2 ^ q - 1 →
        orderOf ((p : ℕ) : ZMod q) = 1 ∧ orderOf ((2 : ℕ) : ZMod p) = q := @ErdosProblems.Erdos249.PaperCompleteR21.mersenneLayer_orderConsumer_instance

theorem mersenneLayer_prime_divisor_order {q p : ℕ} (hq : q.Prime) (hp : p.Prime)
    (hdvd : p ∣ 2 ^ q - 1) :
    orderOf ((2 : ℕ) : ZMod p) = q ∧ q ∣ p - 1 := @ErdosProblems.Erdos249.PaperCompleteR21.mersenneLayer_prime_divisor_order q p hq hp hdvd

theorem mersenneLayer_unbounded_prime_support (B N₀ : ℕ) :
    ∃ q p : ℕ, q.Prime ∧ N₀ ≤ q ∧ p.Prime ∧ p ∣ 2 ^ q - 1 ∧ B < p := by
  apply ErdosProblems.Erdos249.PaperCompleteR21.mersenneLayer_unbounded_prime_support <;> assumption

theorem nine_tenths_lt_cos_pi_div_eight : (9 / 10 : ℝ) < Real.cos (Real.pi / 8) := @ErdosProblems.Erdos249.PaperCompleteR21.nine_tenths_lt_cos_pi_div_eight

theorem one_div_den_le_abs_int_combination (p : ℚ) (m z : ℤ)
    (hne : (m : ℝ) * (p : ℝ) - (z : ℝ) ≠ 0) :
    (1 : ℝ) / (p.den : ℝ) ≤ |(m : ℝ) * (p : ℝ) - (z : ℝ)| := @ErdosProblems.Erdos249.PaperCompleteR21.one_div_den_le_abs_int_combination p m z hne

theorem one_div_den_mul_den_le_abs_diff {x u : ℚ} (hne : x ≠ u) :
    (1 : ℝ) / ((x.den : ℝ) * (u.den : ℝ)) ≤ |(x : ℝ) - (u : ℝ)| := @ErdosProblems.Erdos249.PaperCompleteR21.one_div_den_mul_den_le_abs_diff x u hne
...
theorem orderConsumer_finite_prime_escape {C : ℕ → ℕ} {m d : ℕ}
    ...
    ∀ S : Finset ℕ, ∃ Q₀ : ℕ, ∀ q : ℕ, q.Prime → Q₀ ≤ q →
      ∀ p ∈ S, p.Prime → ¬ p ∣ C (m * q) := @ErdosProblems.Erdos249.PaperCompleteR21.orderConsumer_finite_prime_escape C m d hm horder
...
theorem orderConsumer_index_lt_pow {C : ℕ → ℕ} {m d q p : ℕ}
    ...
    m * q < p ^ d := @ErdosProblems.Erdos249.PaperCompleteR21.orderConsumer_index_lt_pow C m d q p horder hq hp hpC
...
theorem orderConsumer_unbounded_prime_divisors {C : ℕ → ℕ} {m d : ℕ}
    ...
    (horder : BoundedDegreeOrderConsumer C m d) :
    ∀ B N₀ : ℕ, ∃ q p : ℕ,
      q.Prime ∧ N₀ ≤ q ∧ p.Prime ∧ p ∣ C (m * q) ∧ B < p := @ErdosProblems.Erdos249.PaperCompleteR21.orderConsumer_unbounded_prime_divisors C m d hm hlayer horder

theorem orderOf_two_mod_seven : orderOf ((2 : ℕ) : ZMod 7) = 3 := @ErdosProblems.Erdos249.PaperCompleteR21.orderOf_two_mod_seven
...
theorem primeJumpSharpRadius_formula (H p L : ℕ) :
    primeJumpSharpRadius H p L = 3 * p * H + (p + 1) * (L + 2) := @ErdosProblems.Erdos249.PaperCompleteR21.primeJumpSharpRadius_formula H p L

theorem primeJumpSharpRadius_lt_twoCellRadius {H p L : ℕ} (hpH : 0 < p * H) :
    primeJumpSharpRadius H p L < (4 * p * H + (p + 1) * (L + 2) : ℤ) := @ErdosProblems.Erdos249.PaperCompleteR21.primeJumpSharpRadius_lt_twoCellRadius H p L hpH

theorem primeJumpSharpRadius_saves_pH (H p L : ℕ) :
    (4 * p * H + (p + 1) * (L + 2) : ℤ) - primeJumpSharpRadius H p L = p * H := @ErdosProblems.Erdos249.PaperCompleteR21.primeJumpSharpRadius_saves_pH H p L

theorem rational_gap_lower_bound {lo hi : ℚ} (hlt : lo < hi) :
    (1 : ℝ) / ((hi.den : ℝ) * (lo.den : ℝ)) ≤ (hi : ℝ) - (lo : ℝ) := @ErdosProblems.Erdos249.PaperCompleteR21.rational_gap_lower_bound lo hi hlt
...
theorem rough_integer_prime_count_and_totient_bound {a n : ℕ} (ha : 8 ≤ a)
    ...
    n.primeFactors.card < 2 ^ a / 4
      ∧ (3 / 4 : ℚ) * (n : ℚ) < (Nat.totient n : ℚ) := @ErdosProblems.Erdos249.PaperCompleteR21.rough_integer_prime_count_and_totient_bound a n ha hnPos hrough hnPow

theorem shortWindow_inequalities :
    (23 : ℕ) < 2 * 2 ^ 4 ∧ (93 : ℕ) < 2 * 2 ^ 6 := @ErdosProblems.Erdos249.PaperCompleteR21.shortWindow_inequalities

theorem sum_divisors_totient_ne_totient :
    (∀ n : ℕ, ∑ d ∈ n.divisors, Nat.totient d = n) ∧
      (∑ d ∈ (2 : ℕ).divisors, Nat.totient d) = 2 ∧ Nat.totient 2 = 1 := @ErdosProblems.Erdos249.PaperCompleteR21.sum_divisors_totient_ne_totient

theorem three_not_dvd_two_pow (j : ℕ) : ¬ (3 ∣ 2 ^ j) := @ErdosProblems.Erdos249.PaperCompleteR21.three_not_dvd_two_pow j

theorem totientBlock_concatenation (a b N : ℕ) :
    totientBlock (a + b) N = 2 ^ b * totientBlock a N + totientBlock b (N + a) := @ErdosProblems.Erdos249.PaperCompleteR21.totientBlock_concatenation a b N
