...
/-!
# Palomar challenge for Erdős problem #68, band a
...
noncomputable def strictFacTopRat (x : ℚ) (n : ℕ) : ℤ :=
  ⌊(n.factorial : ℚ) * x⌋ + 1
/-- States res:carry-characterization, res:strict-successor-complete-characterization from the short record for Erdős problem #68. Transported from ErdosProblems.Erdos68.PaperComplete.carry_characterisation in the substantive development, whose statement was refereed against the paper in the coverage ledger. -/
...
theorem companion_orbit :
  ...
  sorry
/-- States eq:companion-cofinal, res:companion-orbit-rationality-boundary from the short record for Erdős problem #68. Transported from ErdosProblems.Erdos68.PaperComplete.companion_orbit_boundary in the substantive development, whose statement was refereed against the paper in the coverage ledger. -/
theorem companion_orbit_boundary :
    ...
    (Irrational factorialGapSeries ↔
      ∀ B : ℕ, ∃ m : ℕ, B < m ∧
        (facFloor companionConstant m + 2) % (m : ℤ) ≠ 0) := by
  sorry
/-- States eq:channel-basis-expansion, res:divisor-channel-coordinates from the short record for Erdős problem #68. Transported from ErdosProblems.Erdos68.PaperComplete.divisor_channel_coordinates in the substantive development, whose statement was refereed against the paper in the coverage ledger. -/
theorem divisor_channel_coordinates :
  ...
  sorry
/-- States res:finite-channel-moment-certificate from the short record for Erdős problem #68. Transported from ErdosProblems.Erdos68.PaperComplete.finite_channel_moment_certificate in the substantive development, whose statement was refereed against the paper in the coverage ledger. -/
theorem finite_channel_moment_certificate {D p : ℕ}
  ...
  sorry
/-- States res:finite-channel-moment-certificate from the short record for Erdős problem #68. Transported from ErdosProblems.Erdos68.PaperComplete.finite_channel_moment_certificate_eq in the substantive development, whose statement was refereed against the paper in the coverage ledger. -/
theorem finite_channel_moment_certificate_eq {D p G : ℕ}
    (hD : 2 ≤ D) (hp : p.Prime) (hDp : D / 2 < p) (hpD : p ≤ D)
    (hG : IsScalarTailGcd D G) :
    G = finiteScalarGcd D (D * (2 * p - 1)) := by
  sorry
/-- States eq:global-complementary-target, res:global-complementary-criterion from the short record for Erdős problem #68. Transported from ErdosProblems.Erdos68.PaperComplete.global_complementary_criterion_nat in the substantive development, whose statement was refereed against the paper in the coverage ledger. -/
theorem global_complementary_criterion_nat
  ...
  sorry
/-- States long68:eq:global-scale, long68:res:global-residue from the long record for Erdős problem #68. Transported from ErdosProblems.Erdos68.PaperComplete.global_complementary_criterion_prime in the substantive development, whose statement was refereed against the paper in the coverage ledger. -/
