...
/-!
# Erdős #257, record sections 6 to 9: detailed results and their hypotheses; which hypotheses remain unproved (part 4 of 4)
...
noncomputable def seamResetDeviation (r : ℕ) : ℤ :=
  (seamIntegerGreedyRemainder (r + 1) : ℤ) -
    ((2 ^ (r + 1) : ℕ) : ℤ)
/-- States the paper statement it is bound to from the long record for Erdős problem #257. Transported from ErdosProblems.Erdos257.PaperCompleteR21.paper_theoremA_right_branch_forces_small_deviation in the substantive development, whose statement was refereed against the paper in the coverage ledger. -/
theorem paper_theoremA_right_branch_forces_small_deviation
  ...
  sorry
/-- States record:257bm-i13 from the long record for Erdős problem #257. Transported from ErdosProblems.Erdos257.PaperCompleteR21.paper_two_sided_dyadic_bound in the substantive development, whose statement was refereed against the paper in the coverage ledger. -/
theorem paper_two_sided_dyadic_bound
    ...
    min (seamIntegerGreedyRemainder s) ((seamAdjacentCut s hs).overshoot) ≤
      2 ^ s := by
  sorry
/-- States record:257bm-i14 from the long record for Erdős problem #257. Transported from ErdosProblems.Erdos257.PaperCompleteR21.paper_universal_band_condition_unfolded in the substantive development, whose statement was refereed against the paper in the coverage ledger. -/
