...
problem is open and the first resistant explicit base is `3/2`. The rational-base
family proves irrationality on the strict contour `log b / log a < C0 / C1`.

## What is stated here

* `RationalBaseRegion`. Irrationality of the literal `F(a/b)` on the exact
  strict contour, with an irrationality-exponent bound uniform over positive
  integral powers. In particular `F((31/4)^r)` is irrational and has exponent
  less than `2981509/9909` for every positive natural `r`. The approximation
  forms, positivity and rates are supplied in the source proof.
