...
/-!
# Erdős #257, record sections 6 to 10: detailed results and their hypotheses; hypothesis-specific obstructions (part 2 of 3)
...
namespace PalomarCorpus.E257_23.Shared
...
end PalomarCorpus.E257_23.Shared

namespace PalomarCorpus.E257.PaperStatementsAC
open Finset
export PalomarCorpus.E257_23.Shared (CriticalDyadicBandIndex DyadicBandEscape)
...
namespace PalomarCorpus.E257.PaperStructuresBZ
open Finset
open scoped BigOperators
open Set
open Filter
...
/-- Local definition seamUpperResetCharge, copied so the compared statements of this entry elaborate against Mathlib alone. -/
noncomputable def seamUpperResetCharge (d : ℕ) (hd5 : 5 ≤ d) : ℕ :=
  4 * (seamAdjacentCut d hd5).overshoot +
    (seamAdjacentCut d hd5).abovePulse
/-- Local definition SeamUpperResetCriticalBandEscape, copied so the compared statements of this entry elaborate against Mathlib alone. -/
      ...
      ∀ j : ℕ, j ≤ d →
        2 ^ (d - j + 1) <
          ...
          4 * (seamAdjacentCut d hd5).overshoot +
            ...
            2 ^ (d - j + 1)
/-- States thm:critical-dyadic-band from the long record for Erdős problem #257. Transported from Erdos249257.HalfUpperResetCriticalBand.seamUpperResetCriticalBandEscape_iff in the substantive development, whose statement was refereed against the paper in the coverage ledger. -/
theorem seamUpperResetCriticalBandEscape_iff :
    SeamUpperResetCriticalBandEscape ↔ SeamUpperResetDyadicBandEscape := by
  sorry
/-- States prop:critical-band-index from the long record for Erdős problem #257. Transported from ErdosProblems.Erdos257.PaperCompleteR21.paper_critical_band_index_collapse in the substantive development, whose statement was refereed against the paper in the coverage ledger. -/
theorem paper_critical_band_index_collapse :
    (∀ d E : ℕ, DyadicBandEscape d E ↔
      ...
      (SeamUpperResetCriticalBandEscape ↔ SeamUpperResetDyadicBandEscape) := by
  sorry
end PalomarCorpus.E257.PaperStructuresBZ
